...
async function updateStopsTimes() {
  try {
    ...
    const activePtResponse = await fetch('active_schedule.json');
    const activePtData = await activePtResponse.json();

    // 3. Определяем нужный файл по active
    const active_petrovac = activePtData.active_petrovac;
    const fileMap = {
      '1bus': 'budva-petrovac_1bus.json',
      '2buses': 'budva-petrovac_2buses.json',
      '3buses': 'budva-petrovac_3buses.json'
    };
    const scheduleFile = fileMap[active_petrovac];
    if (!scheduleFile) {
      console.error('Unknown active buses:', active_petrovac);
      return;
    }
    ...
    const ids = [
      ...
      'Adrovich-s', 'Blizikuche-s', 'Monastery-s', 'Giardino-s', 'Rezhevichi-s',
      'Station-s', 'Petrovac-s', 'Buljarica-s'
    ];
            ...
            document.getElementById('intervalDropdown').addEventListener('click', function handleClick(event) {
                if (event.target.tagName === 'A') {
                    ...
                    fetch('active_schedule.json')
                        ...
                        .then(activeData => {
                            let scheduleFile = '';
                            switch (activeData.active_petrovac) {
                                case '1bus': scheduleFile = 'budva-petrovac_1bus.json'; break;
                                case '2buses': scheduleFile = 'budva-petrovac_2buses.json'; break;
                                case '3buses': scheduleFile = 'budva-petrovac_3buses.json'; break;
                                default: return;
                            }
